...
class BattlePass:
    ...
    def update(self, dt, game_events=None):
        ...
        if self.weekly_reset_timer >= 604800:
            self.weekly_reset_timer = 0
            self.challenges["weekly"] = random.sample(
                QUEST_CHALLENGES["weekly"], min(3, len(QUEST_CHALLENGES["weekly"])))
    ...
    def draw(self, screen, player=None):
        ...
        for cat in categories:
            ...
            for ch in challenges:
                ...
                pygame.draw.rect(screen, season_data["color"], (prog_bar_x, ch_y +
                                 2, int(prog_w * prog_ratio), 10), border_radius=5)
